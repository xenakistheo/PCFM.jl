...
Dataset for 1D Burgers equation solutions loaded from an HDF5 file.
...
"""
    Burgers1DDataset(root, split, data_file)

Open the HDF5 file at `joinpath(root, data_file)` and return a `Burgers1DDataset`.
"""
function Burgers1DDataset(root::String, split::String, data_file::String)
    ...
    n_data = N_ic * N_bc
    return Burgers1DDataset(root, split, file, u, N_ic, N_bc, Nx, nt, n_data)
end

Base.length(ds::Burgers1DDataset) = ds.n_data

"""
    getindex(ds::Burgers1DDataset, index::Int)

Return the solution array for sample `index` (1-based) as Float32 of shape (Nx, nt).
"""
function Base.getindex(ds::Burgers1DDataset, index::Int)
...
Base.close(ds::Burgers1DDataset) = close(ds.file)
