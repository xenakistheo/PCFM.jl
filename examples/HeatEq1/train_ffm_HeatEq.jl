"""
Train an FFM model on the 1D heat (diffusion) equation and save a checkpoint.

Assumes the training dataset already exists. Run generate_data_heat.jl first if needed.

Saves checkpoint to: examples/checkpoints/ffm_heat_1_checkpoint.jld2
...
train_file  = joinpath(data_dir, "heat_train_n6400.h5")
...
# ---------------------------------------------------------------------------
println("=" ^ 60)
println("Heat (Diffusion) Equation — Train FFM")
println("=" ^ 60)

# 1. Load training batch
if !isfile(train_file)
    error("Training data not found at $train_file.\nRun examples/HeatEq1/generate_data_heat.jl first.")
end
println("\n[1/4] Loading training batch from $train_file ...")
u_data = load_heat_batch(train_file, batch_size)
println("  Data shape: $(size(u_data))  — (nx, nt, 1, batch_size)")

# 2. Create model
println("\n[2/4] Creating FFM (Functional Flow Matching) model...")
ffm = FFM(
    nx = nx,
    nt = nt,
...
p1 = Plots.plot(1:length(losses), losses,
    ...
    title = "Diffusion — Training Loss", legend = false, linewidth = 2)
display(p1)
