...
data_path = joinpath(@__DIR__, "..", "..", "final_samples", "samples_burgers_IC_IC_Mass_Flux_k5_soltol_e5.jld2")
...
#   CE(τ) = (1/N) Σₙ ‖R_τ(û⁽ⁿ⁾)‖₂
# with residuals mirroring burgers_constraints_IC_Mass_Flux! in src/constraints.jl (k=5):
...
function burgersICMassFlux_constraint_errors(samples, u0_ic, nx, nt, dx, dt; k=5, eps=1e-6)
...
ces          = [burgersICMassFlux_constraint_errors(s, u0_ic, nx, nt, dx, dt; k=5) for s in all_samples]
...
println(rpad("Solver", 20), rpad("IC", 14), rpad("Mass (CL)", 14), "Godunov(k=5)")
