# Inference script for the 1D Burgers equation (BC + Mass regime) using Alaina's
# projection-based sample_pcfm (solve_projection interface).
#
# Mirrors infer_burgers_BC.jl: same model, same IC, same save format.
# Solvers: BurgersBCMassSolver (LBFGS) + BurgersBCMassIPSolver (IPNewton)
#
...
SAMPLES_PATH = length(ARGS) >= 2 ? ARGS[2] : "samples_burgers_BC_alaina.jld2"
...
# ---------------------------------------------------------------------------
println("=" ^ 60)
println("Burgers BC — Functional Flow Matching (Alaina solvers)")
println("=" ^ 60)
